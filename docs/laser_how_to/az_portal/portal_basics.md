...
Each level in the hierarchy typically opens a new navigation menu on the left-hand side, or replaces one previously visible. To collapse a menu, select its collapse arrow, which is highlighted in red circles.
...
Now, from the LASER subscription page, scroll down the navigation menu and select Resource groups, as shown:
...
The page will then list the resource groups you can access. Each resource group is a different VRE. The resource group ID contains your VRE's ID, e.g. p0000v01. The full resource group ID will take the form `UoL-uks-LRDP-<vre id>-prod-rg`. Select a resource group for one of your VREs and follow the next pages to see what VRE information you can monitor using the portal.
